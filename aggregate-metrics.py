...
def write_throughput_all_experiments(all_experiments_metrics):
    with open(f'throughput.csv', 'w') as f:
        for exp_num, exp_metrics in all_experiments_metrics.items():
            throughput_aggregates = [exp_metrics['min'][2], exp_metrics['avg'][2], exp_metrics['max'][2]]
            result = [str(exp_num)]
    ...
    with open(f'all_metrics.csv', 'w') as f:
        for exp_num, exp_metrics in all_experiments_metrics.items():
            ...
            for i in range(len(exp_metrics['min'])):
                aggregates = [exp_metrics['min'][i], exp_metrics['avg'][i], exp_metrics['max'][i]]
            ...
            if file.endswith('.metrics'):
                ...
                with open(f'{folder}/{file}', 'r') as f:
                    ...
                    if len(sum_metrics) == 0:
                        ...
                        aggregate_metrics['min'] = [min(tup) for tup in zip(aggregate_metrics['min'], metrics_float)]
                        aggregate_metrics['max'] = [max(tup) for tup in zip(aggregate_metrics['max'], metrics_float)]
                        sum_metrics = [sum(tup) for tup in zip(sum_metrics, metrics_float)]
        aggregate_metrics['avg'] = [x / count for x in sum_metrics]
        
        all_experiments_metrics[index] = aggregate_metrics
        
        # # Write each aggregate to its own file
        # for key, value in aggregate_metrics.items():
        #     with open(f'{folder}/{key}.metrics.aggregate', 'w') as f:
        #         f.write(','.join(["{:.2f}".format(x) for x in value]))
                
        # # Write all aggregates into one file, separated by \n
        # with open(f'{folder}/all.metrics.aggregate', 'w') as f:
        #     for key, value in aggregate_metrics.items():
        #         f.write(','.join(["{:.2f}".format(x) for x in value]))
        #         f.write('\n')
        ...
        for index, folder in enumerate(experiment_folders):
            ...
            for i in range(1, nc + 1):
                with open(f'{folder}/{i}.metrics', 'r') as r:
                    ...
                    metrics_float = [float(x) for x in metrics]
                    metrics_rounded = ["{:.2f}".format(x) for x in metrics_float]
